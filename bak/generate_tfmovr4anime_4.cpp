...
// 指定された開始フレームと絶対位相から、正しいパターン名(ccppc等)を生成する
static std::string generate_pattern_name(size_t start_frame, CyclePhase phase) {
    ...
    for (int j = 0; j < 5; ++j) {
        ...
        if (current_rem == p1 || current_rem == p2) {
            pattern[j] = 'p';
        }
    }
    return pattern;
...
static CyclePhase detect_dominant_phase(std::span<const TfmEntry> window, CyclePhase current) {
    ...
    // 動きが極端に少ない場合は、現在の判断を維持する（汎用的な静止画対策）
    ...
    // 動きの密度に応じて要求される支配率を動的に変更 (Density-based confidence)
    // 動きが多い(p>25)なら0.7で敏感に、少ないなら0.9以上を要求
    ...
    // 強力な慣性：現在のフェーズが50%以上の整合性を持つなら、変更を極力抑える
    if (current != CyclePhase::Unknown) {
        int current_err = count_errors(window, current);
        double current_acc = 1.0 - (static_cast<double>(current_err) / total_valid_p);
        if (current_acc > 0.5) return current;
    }
...
static void print_usage() {
    std::cout << "--------------------------------------" << std::endl;
    std::cout << "generate_tfmovr4anime v1.2.0 by Ikotas" << std::endl;
    std::cout << "--------------------------------------" << std::endl;
    std::cout << "Usage: generate_tfmovr4anime.exe TFM_output_file(*.tfm)" << std::endl << std::endl;
