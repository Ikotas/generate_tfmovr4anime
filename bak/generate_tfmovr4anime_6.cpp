...
static CyclePhase detect_dominant_phase(std::span<const TfmEntry> window, CyclePhase current) {
    ...
    for (int i = 0; i < 5; ++i) {
        // 算術オーバーフロー防止のため、加算前にキャスト
        int64_t sum = counts[static_cast<size_t>(i)] + counts[static_cast<size_t>((i + 1) % 5)];
        if (sum > max_sum) {
            max_sum = sum;
            best_phase_idx = i;
        }
...
static size_t find_boundary(std::span<const TfmEntry> window, CyclePhase old_p, CyclePhase new_p, size_t last_boundary) {
    size_t best_t = 0;
    int64_t min_error = 2000000000; // 十分に大きな値
    ...
    for (size_t i = 0; i < entries.size(); i += 5) {
        ...
        if (detected != CyclePhase::Unknown && detected != current_phase) {
            if (current_phase == CyclePhase::Unknown) {
                ...
                // 検証済みの200フレームルックバックを適用
                size_t b_lookback = 200;
                size_t b_start = (i < b_lookback) ? 0 : i - b_lookback;
                size_t b_end = std::min(entries.size(), i + window_size / 2);
